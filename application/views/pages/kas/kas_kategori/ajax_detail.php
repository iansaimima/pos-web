...
if (isset($detail) && is_array($detail) && count($detail) > 0) {
    ...
    $alur_kas = strtolower($detail["alur_kas"]);
...
?>
<form class="form-horizontal" onsubmit="return false;" id="form-kas-kategori-detail">
    ...
    <div class="form-group row mb-2">
        <label class="col-form-label col-sm-4">Nama</label>
        <div class="col-sm-8">
            <select name="alur_kas" class="form-control input-sm">
                <option value="" selected disabled>Pilih Alur Kas</option>
                <option value="masuk" <?= $alur_kas == "masuk" ? "selected" : "" ?>>Masuk</option>
                <option value="keluar" <?= $alur_kas == "keluar" ? "selected" : "" ?>>Keluar</option>
            </select>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        $("#modal-kas-kategori-title").html('<?= $title ?>');
    });

    function save() {
        var form_data = $("#form-kas-kategori-detail").serializeArray();
        ajax_post(
            '<?= base_url("admin/kas_kategori/ajax_save") ?>',
            form_data,
            function(resp) {
                try {
